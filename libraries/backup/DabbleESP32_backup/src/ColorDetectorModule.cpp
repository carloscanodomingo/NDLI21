...
void ColorDetectorModule::processData()
{
	...
	Serial.println("After processing data");
	...
	Serial.println("After getting function id");
	 if(functionID == COLOR_DATA )
	{
		...
		{
			Serial.println("Get dabble Instance");
		...
		Serial.println("Not second line");
		currentArgnumber=getDabbleInstance().getArgumentNo();
		Serial.println(currentArgnumber);
		Serial.println(getDabbleInstance().getArgumentLength(1));
		for (int i = 1; i < currentArgnumber; i++) //color values comes from second line
          {
		  byte * ArgumentData = getDabbleInstance().getArgumentData(i);
		  Serial.println((int)ArgumentData, HEX);
		  if (ArgumentData == NULL)
		  {
			  Serial.println("Im going to explode!!!!!!!!!!!!!!!!!");
			  break;
		  }
            for (int j = 0; j < 3; j++)
               {
	 ...
	 Serial.println("FInishing processing");
}
